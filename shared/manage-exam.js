// Global variables
    ...
    return;
        ...
        /*function extractUrlParameters() {
            ...
            // Update display
            document.getElementById('teacherId').textContent = currentParams.teacherId || 'Not specified';
...
function extractUrlParameters() {
    ...
    // Update display - Fix the element ID
    document.getElementById('teacherIdDisplay').textContent = currentParams.teacherId || 'Not specified';
}
...
/*async function loadExams() {
    ...
    try {
        ...
        const teacherExamsResponse = await fetch(`/api/exams?teacherId=${encodeURIComponent(currentParams.teacherId)}`, {
            method: 'GET',
            headers: {
        ...
        let teacherExams = [];
        if (teacherExamsResponse.ok) {
            teacherExams = await teacherExamsResponse.json();
        }

        // Try to load default exams from API, fallback to hardcoded
        let defaultExams = [defaultExamData];
        try {
            const defaultExamsResponse = await fetch(`/api/exams/default`, {
                method: 'GET',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${localStorage.getItem('token')}`
            ...
            if (defaultExamsResponse.ok) {
                const apiDefaultExams = await defaultExamsResponse.json();
                if (apiDefaultExams.length > 0) {
                    defaultExams = apiDefaultExams.map(exam => ({
                        ...exam,
                        isDefault: true,
                        teacherId: 'Default'
                    }));
            ...
            console.log('Using hardcoded default exam data');
        }

        // Combine both arrays
        allExams = [...teacherExams, ...defaultExams];
        filteredExams = [...allExams];
        
        console.log('Loaded exams:', allExams.length, '(Teacher:', teacherExams.length, ', Default:', defaultExams.length, ')');
        displayExams(filteredExams);

    } catch (error) {
        console.error('Error loading exams:', error);
        
        // Fallback: use only default exam
        allExams = [defaultExamData];
        filteredExams = [...allExams];
        displayExams(filteredExams);
    }
}*/
async function loadExams() {
    ...
    try {
        ...
        // Only try to load teacher exams if teacherId is provided
        if (currentParams.teacherId && currentParams.teacherId !== '') {
            try {
                const teacherExamsResponse = await fetch(`/api/exams?teacherId=${encodeURIComponent(currentParams.teacherId)}`, {
                    method: 'GET',
                    headers: {
                ...
                console.log('Error loading teacher exams, continuing with default exam only:', error);
            }
        }

        // Always include default exam
        allExams = [...teacherExams, defaultExamData];
        filteredExams = [...allExams];
        
        console.log('Loaded exams:', allExams.length, '(Teacher:', teacherExams.length, ', Default: 1)');
        displayExams(filteredExams);
...
function displayExams(exams) {
    ...
    container.innerHTML = exams.map(exam => {
        ...
        const isDefault = exam.isDefault || false;
        
        return `
            <div class="exam-card ${isDefault ? 'default-exam' : ''}" data-exam-id="${examId}">
                <div class="exam-header">
                    <h3 class="exam-title">
                        ${isDefault ? '<i class="fas fa-star text-warning me-2" title="Default Exam"></i>' : ''}
                ...
                <div class="exam-meta">
                    ...
                    <div class="meta-item">
                        <div>Type: <span class="meta-value ${isDefault ? 'text-warning' : ''}">${isDefault ? 'Default' : 'Custom'}</span></div>
                    </div>
                    <div class="meta-item">
                        <div>Teacher ID: <span class="meta-value">${exam.teacherId}</span></div>
                    </div>
                </div>
                ...
                <div class="exam-actions">
                    <button class="btn btn-primary btn-sm" onclick="manageQuestions('${examId}')">
                        <i class="fas fa-eye me-2"></i>${isDefault ? 'View Questions' : 'Manage Questions'}
                    </button>
                    ${!isDefault ? `
                    ...
                    ` : `
                        <button class="btn btn-outline-success btn-sm" onclick="cloneExam('${examId}')">
...
// Updated editExam function
function editExam(id) {
    const exam = allExams.find(e => (e.id === id || e._id === id));
    if (!exam) return;
    
    // Prevent editing default exams
    if (exam.isDefault) {
        alert('Cannot edit default exams. Please clone the exam first to create your own copy.');
        return;
    }
...
async function deleteExam(examId, examTitle) {
    ...
    // Prevent deleting default exams
    if (exam && exam.isDefault) {
        alert('Cannot delete default exams.');
        return;
    }
...
// Update saveExam function - remove category, collegeId, branch
async function saveExam() {
    ...
    const examData = {
        ...
        status,
        description,
        teacherId: currentParams.teacherId
    };
    ...
    const exam = allExams.find(e => (e.id === examId || e._id === examId));
    const isDefaultExam = exam && exam.isDefault;
    
    document.getElementById('questionModalTitle').textContent = `${isDefaultExam ? 'View' : 'Manage'} Questions - ${exam.title}`;
    
    // Hide/show add question button based on exam type
    const addQuestionBtn = document.querySelector('#questionModal .btn-success');
    if (addQuestionBtn) {
        addQuestionBtn.style.display = isDefaultExam ? 'none' : 'block';
    }
...
async function loadQuestions(examId) {
    try {
        // Handle default exam with hardcoded questions
        if (examId === 'default-class7-science') {
            questions = [
                {
                    id: 1,
        ...
        // Regular API call for other exams
        const token = localStorage.getItem('token');
...
function displayQuestions() {
    ...
    const exam = allExams.find(e => (e.id === currentExamId || e._id === currentExamId));
    const isDefaultExam = exam && exam.isDefault;
    ...
    if (!questions || questions.length === 0) {
        questionsList.innerHTML = `
            <div class="text-center">
                <p class="text-muted">No questions added yet. ${!isDefaultExam ? 'Click "Add New Question" to get started.' : ''}</p>
            </div>
        `;
        return;
    }
                ...
                <div class="d-flex justify-content-between align-items-start">
                    <div class="flex-grow-1">
                        <h6>
                            ...
                            ${isDefaultExam ? '<i class="fas fa-star text-warning ms-2" title="Default Question"></i>' : ''}
                        </h6>
                        ${questionContent}
                    ...
                    <div class="ms-3">
                        ...
                        ` : `
                            <span class="text-muted small">Default questions are read-only</span>
                        `}
                    </div>
                </div>
            </div>
...
// Clone default exam to create teacher's own copy
async function cloneExam(examId) {
    const exam = allExams.find(e => (e.id === examId || e._id === examId));
    if (!exam || !exam.isDefault) {
        alert('Can only clone default exams');
        return;
    }

    const newTitle = prompt(`Enter title for cloned exam:`, `${exam.title} (Copy)`);
    if (!newTitle) return;

    try {
        const response = await fetch(`/api/exams/clone/${examId}`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${localStorage.getItem('token')}`
        ...
        if (response.ok) {
            const result = await response.json();
            console.log('Exam cloned successfully:', result);
            loadExams();
            alert('Exam cloned successfully! You can now edit and customize it.');
        } else {
            const errorData = await response.json();
            alert('Error cloning exam: ' + (errorData.message || 'Failed to clone exam'));
        }
    } catch (error) {
